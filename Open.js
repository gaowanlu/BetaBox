...
function OpenJs(dom){//@OpenJs对象构造器
    ...
    //嵌套渲染DOM树
    function OPENJS_VIEW(father){
        if(father.OPENJS_child){
            for(let key in father.OPENJS_child){
                father.appendChild(father.OPENJS_child[key]);
                //进行递归
                OPENJS_VIEW(father.OPENJS_child[key]);
            }
        }
    }
